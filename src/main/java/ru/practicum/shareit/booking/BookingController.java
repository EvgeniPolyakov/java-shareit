...
public class BookingController {
    ...
    public BookingDto add(@Valid @RequestBody BookingDto bookingDto) {
        log.info("Получен запрос POST (createBooking). Добавлено бронирование: {}", bookingDto);
        Booking booking = BookingMapper.toBooking(bookingDto);
        return BookingMapper.toBookingDto(bookingService.add(booking));
    }

    @PatchMapping("/{id}")
    public BookingDto update(@PathVariable("id") Long id, @Valid @RequestBody BookingDto bookingDto) {
        Booking booking = BookingMapper.toBooking(bookingDto);
